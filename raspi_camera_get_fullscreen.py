...
cap = cv2.VideoCapture(0) # use CAP_DSHOW in laptop, to remove delay when accessing the camera
...
async def main():
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if rectangle_x < face_center_x < rectangle_x + rectangle_width and rectangle_y < face_center_y < rectangle_y + rectangle_height and delay == 0:
                # Face is completely inside the rectangle, send full frame
                frame_processing_thread = FrameProcessingThread(raw_img, url="http://192.168.43.64:8000/api/face_detection")
